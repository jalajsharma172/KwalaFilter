...
export default function EventListener() {
  ...
  const handleStartListening = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = `/listen?address=${encodeURIComponent(address.trim())}&topic0=${encodeURIComponent(topic0.trim())}&abi=${encodeURIComponent(abiBase64)}`;
            ...
            <CardContent className="space-y-4">
              <form onSubmit={handleStartListening} className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="address">Contract 123 Address</Label>
                  <Input
                    id="address"
...
//             <span className="text-muted-foreground">Tx Hash:</span>{' '}
//             <code className="text-primary break-all cursor-pointer flex items-center gap-2 group">
//               <span><u>{data.transactionHash.substring(0, 20)}</u>...</span>
//               <ExternalLink
//                 className="h-3 w-3 opacity-0 group-hover:opacity-100 transition"
//                 onClick={() =>
...
//                 {data.topics.map((topic: string, idx: number) => (
//                   <div key={idx} className="text-muted-foreground">
//                     [{idx}] <code className="text-foreground text-xs break-all">Encoded Data -{topic}</code>
//                     [{idx}] <code className="text-foreground text-xs break-all">Decoded Data is -{topic}</code>
//                   </div>
//                 ))}
//               </div>
//             </div>
