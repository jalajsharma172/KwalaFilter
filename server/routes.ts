...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/subscriptions', async (req, res) => {
    ...
    const { address, blocknumber, topic0, abi, api, params, times, ActionName, ActionType, TargetFunction, TargetFunctionParameters, TargetContract, Workflow_Owner } = req.body || {};
  ...
  // GET - Fetch YAML workflows from Supabase (simple)
  app.get('/api/yaml', async (_req, res) => {
    try {
  ...
  // POST - Save YAML workflow to Supabase
